...
if (count($arResult["ERRORS"]) > 0):
	...
	<div class="notification is-danger is-light auth-error-message">
		<h3><?php
			ShowError(implode("<br>", $arResult["ERRORS"])); ?></h3>
	</div>
	...
	<form method="post" class="box box-reg" action="<?= POST_FORM_ACTION_URI ?>" name="regform" enctype="multipart/form-data">
		...
		foreach ($arResult["SHOW_FIELDS"] as $FIELD): ?>
			...
			<label class="label reg-value-name"><?= GetMessage("REGISTER_FIELD_" . $FIELD) ?>:
				<?
				if ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y"): ?><span class="starrequired">*</span><?
				endif ?>
			</label>
			...
			if ($FIELD === "PASSWORD" || $FIELD === "CONFIRM_PASSWORD"): ?>
				<div class="control">
					<input class="input" type="password" name="REGISTER[<?= $FIELD ?>]"
						   value="<?= $arResult["VALUES"][$FIELD] ?>" placeholder="<?= GetMessage(
			...
			<? elseif ($FIELD === "PERSONAL_NOTES"): ?>
				<div class="control">
					<textarea class="textarea" name="REGISTER[<?= $FIELD ?>]" placeholder="<?= GetMessage(
						"REGISTER_INFO_FIELD"
					) ?>"><?= $arResult["VALUES"][$FIELD] ?></textarea>
				</div>
			<? elseif ($FIELD === "PERSONAL_PHOTO"): ?>
				<div class="control">
					<input class="input" type="file" name="REGISTER[<?= $FIELD ?>]" accept="image/png,image/jpg"
						   value="<?= $arResult["VALUES"][$FIELD] ?>" autocomplete="off">
				</div>

			<? elseif ($FIELD === "EMAIL"): ?>
				<div class="control">
					<input class="input" name="REGISTER[<?= $FIELD ?>]" type="email" placeholder="<?= GetMessage(
						"REGISTER_EMAIL_FIELD"
					) ?>" value="<?= $arResult["VALUES"][$FIELD] ?>">
				</div>
			<? else: ?>
				<div class="control">
					<input class="input" type="text" name="REGISTER[<?= $FIELD ?>]" value="<?= $arResult["VALUES"][$FIELD] ?>" placeholder="">
				</div>
			<? endif; ?>
		<? endforeach; ?>
		...
		if ($arResult["USER_PROPERTIES"]["SHOW"] == "Y"): ?>
			<tr>
				<td colspan="2"><?= trim($arParams["USER_PROPERTY_NAME"]) <> '' ? $arParams["USER_PROPERTY_NAME"]
						: GetMessage("USER_TYPE_EDIT_TAB") ?></td>
			</tr>
			<?
		...
		<div class="field is-grouped is-grouped-centered is-reg-buttons">
			<div class="control">
				<input class="button is-link" type="submit" name="register_submit_button" value="<?= GetMessage(
					"AUTH_REGISTER"
				) ?>">
			</div>
			<div class="control">
